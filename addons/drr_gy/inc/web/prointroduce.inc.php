...
if(checksubmit('submit')){
    ...
    if($_GPC['id']==''){                
        ...
        if($res){
            message('添加成功',$this->createWebUrl('prointroduce',array()),'success');
        }else{
            message('添加失败','','error');
        }
    }else{
        ...
        if($res){
            message('编辑成功',$this->createWebUrl('prointroduce',array()),'success');
        }else{
            message('编辑失败','','error');
        }
    }
}
include $this->template('web/prointroduce');