...
if($_GPC['op']=='add'){
    ...
    $res=pdo_update('lhyzhnc_sun_popbanner',array('status'=>2),array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if($res){
        message('通过成功！', $this->createWebUrl('banner'), 'success');
    }else{
        message('通过失败！','','error');
    }
}elseif($_GPC['op']=='nopass'){
    $res=pdo_update('lhyzhnc_sun_popbanner',array('status'=>3),array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if($res){
        message('拒绝成功！', $this->createWebUrl('banner'), 'success');
    }else{
        message('拒绝失败！','','error');
    }
    ...
    $pagesize=10;
    $sql="select * from " . tablename("lhyzhnc_sun_popbanner") .$where." order by sort asc ";
    $total=pdo_fetchcolumn("select count(*) as wname from " . tablename("lhyzhnc_sun_popbanner") .$where." order by sort asc ",$data);
