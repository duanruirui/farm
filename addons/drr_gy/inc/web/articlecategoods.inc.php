...
$template = "web/articlecategoods";

if($_GPC['op']=='add'){
    $template = "web/articlecate_addgoods";
}elseif($_GPC['op']=='save'){
    // $id=intval($_GPC['id']);
    $data['name']=$_GPC['name'];
    $data['img']=$_GPC['img'];
    ...
    if($_GPC['id']==''){
        ...
        if($res){
            message('添加成功',$this->createWebUrl('articlecategoods'),'success');
        }else{
            message('添加失败','','error');
        }
    }else{
        ...
        if($res){
            message('修改成功',$this->createWebUrl('articlecategoods'),'success');
        }else{
            message('修改失败','','error');
        }
    }
    ...
    if($res){
        ...
        message('删除成功！', $this->createWebUrl('articlecategoods'), 'success');
    }else{
        message('删除失败！','','error');
    }
}
    ...
    $data[':uniacid']=$_W['uniacid'];
    $where=" WHERE  num = 2 and  uniacid=".$_W['uniacid'];
