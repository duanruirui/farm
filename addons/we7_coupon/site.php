...
class We7_couponModuleSite extends WeModuleSite {
	...
	public function doWebCardmanage() {
		...
		$setting = $this->settings;
		if ($op == 'display') {
			$actions_permit = we7_coupon_check_user_permit(array('we7_coupon_permission_mc_credit1', 'we7_coupon_permission_mc_credit2'));
	...
	public function doWebCouponmanage() {
		...
		if ($op == 'post') {
			...
			if (checksubmit('submit')) {
				...
				$coupon->setCustomMenu('立即使用', '', murl('entry', array('m' => 'we7_coupon', 'do' => 'consume'), true, true));
		...
		if ($op == 'sync') {
            ...
            $pageindex = max(1, $_GPC['page']);
            $psize = 15;
            $cards = pdo_getall('coupon', array('acid' => $_W['acid'], 'source' => COUPON_TYPE), array('id', 'status', 'card_id', 'acid', 'source'), 'id', 'id DESC', array($pageindex, $psize));
			we7_coupon_activity_coupon_sync($cards);
		...
		if ($params['result'] == 'success' && $params['from'] == 'notify') {
			...
			if (!empty($order)) {
				...
				if ($log['type'] != 'credit') {
					...
					mc_card_grant_credit($uid, $log['card_fee'], $order['store_id']);
				}
