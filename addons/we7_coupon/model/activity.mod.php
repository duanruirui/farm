 ...
 * 同步微信卡券最新状态
 */
function we7_coupon_activity_coupon_sync($cards = array()) {
	global $_W;
	...
	$coupon_api = new coupon($_W['acid']);
	if (empty($cards)) {
	    $cards = pdo_getall('coupon', array('acid' => $_W['acid']), array('id', 'status', 'card_id', 'acid', 'source'));
    }
	foreach ($cards as $val) {
	    if ($val['source'] != WECHAT_COUPON) {
	        continue;
        }
